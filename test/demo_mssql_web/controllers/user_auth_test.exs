defmodule DemoMssqlWeb.UserAuthTest do
  use DemoMssqlWeb.ConnCase, async: true

  alias DemoMssql.Accounts
  alias DemoMssqlWeb.UserAuth
  import DemoMssql.AccountsFixtures

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, DemoMssqlWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
